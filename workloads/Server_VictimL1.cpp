...
extern "C" {
    void m5_qvc_ctrl(uint64_t cmd, uint64_t addr, uint64_t len);
}
...
int main() {
    ...
    run_dijkstra_calculate();
		...
		//Register hot data to the victim cache (the registeration instruction is defined in m5op.)
    std::cout << "Victim Core 0: Registering Hot Sets to QVC..." << std::endl;  
		  // 1. Register 'dist' array
		  // cmd=1 (ADD), addr=dist.data(), len=size in bytes
    m5_qvc_ctrl(1, (uint64_t)dist.data(), dist.size() * sizeof(float));

		  // 2. Register 'adj' array?
		  // Since 'adj' is a vector of vectors (std::vector<std::vector<Edge>>), 
		  // the outer vector only holds headers. The actual data is scattered.
		  // Iterate and register each adjacency list.
		m5_qvc_ctrl(1, (uint64_t)adj.data(), adj.size() * sizeof(std::vector<Edge>));
    for (const auto& edge_list : adj) {
        if (!edge_list.empty()) {
						 volatile Edge e1 = edge_list[0];
             volatile Edge e2 = edge_list[edge_list.size()-1];
             m5_qvc_ctrl(1, (uint64_t)edge_list.data(), edge_list.size() * sizeof(Edge));
        }
    }
